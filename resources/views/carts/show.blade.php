    ...
    <div class="min-h-screen flex flex-col sm:justify-center items-center py-6 sm:pt-0 bg-gray-100">
        <div class="w-full sm:max-w-2xl mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
            ...
            <div class="flex justify-between w-full sm:max-w-2xl">
                <h3 class="sm:max-w-md text-xl">カート内商品一覧</h3>
                <form action="{{ route('carts.delete') }}" method="post">
                    @csrf
                    <x-danger-button class="sm:max-w-md">空にする</x-danger-button>
                </form>
            </div>
        ...
        <div class="flex justify-between w-full sm:max-w-2xl">
            ...
            <x-primary-button onclick="location.href='{{ route('payment.checkout') }}'" class="sm:max-w-md mt-6 px-6 py-4">支払い画面へ</x-primary-button>
        </div>
    </div>
</x-app-layout>
