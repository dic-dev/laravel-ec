            ...
            <table class="mt-4 border-t border-t-gray-200 w-full">
                <thead>
                    <tr class="border-b border-b-gray-200 bg-gray-200">
                        <th>請求No.</th><th>会員No.</th><th>日時</th><th>金額</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($bills as $bill)
                    <tr onclick="location.href='{{ route('admin.bills.show', $bill) }}'" align="center" class="border-b border-b-gray-200 hover:bg-gray-100 cursor-pointer">
                        <td>{{ $bill->id }}</td>
                        <td>{{ $bill->user_id }}</td>
